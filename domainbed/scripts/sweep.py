...
class Job:
    ...
    def __init__(self, train_args, sweep_output_dir, augmentations):
        self.train_args = copy.deepcopy(train_args)
        self.train_args["augmentations"] = augmentations
        args_str = json.dumps(self.train_args, sort_keys=True)
...
def make_args_list(n_trials_from, n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
    data_dir, task, holdout_fraction, single_train_envs, hparams):
    args_list = []
    for trial_seed in range(n_trials_from, n_trials_from+n_trials):
        for dataset in dataset_names:
            for algorithm in algorithms:
                if single_train_envs:
                    all_test_envs = [
                        [j for j in range(datasets.num_environments(dataset)) if j!=i] for i in range(datasets.num_environments(dataset))]
                else:
...
if __name__ == "__main__":
    ...
    if args.command in ['launch', 'delete_incomplete']:
        augmentations_list = [
            ['Original'],
            ['Original', 'IPMix', 'IPMix'],
            ['Original', 'Stylized', 'EdgeDetection',
                'CartoonGAN', 'IPMix', 'IPMix'],
        ]
