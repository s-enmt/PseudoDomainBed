...
def _hparams(algorithm, dataset, random_seed):
    ...
    if dataset in SMALL_IMAGES:
        ...
        _hparam('test_batch_size', 256, lambda r: int(2**r.uniform(3, 9)) )
    elif algorithm == 'ARM':
        _hparam('batch_size', 8, lambda r: 8)
        _hparam('test_batch_size', 64, lambda r: int(2**r.uniform(3, 9)) )
    elif dataset == 'DomainNet':
        _hparam('batch_size', 32, lambda r: int(2**r.uniform(3, 5)) )
        _hparam('test_batch_size', 128, lambda r: int(2**r.uniform(3, 9)) )
    else:
